...
export const getContacts = async ({page = 1, perPage = 10, sortBy = "_id", sortOder = sortList[0]})=> {
  const skip = (page -1) * perPage;
  const items = await ContactCollection.find().skip(skip).limit(perPage).sort({[sortBy]: sortOder});
